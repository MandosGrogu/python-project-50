...
def test_gendiff():
    ...
    assert gen.get_diff(f1_p, f2_p) == tru
    ...
    assert gen.get_diff(f1_json_p, f2_json_p, format_name='stylish') == tru1

    tru_p0 = os.path.join(os.getcwd(), 'tests/test_data/fpath2_gr_tru.json')

    tru0 = json.dumps(json.load(open(tru_p0)), indent=4)
    
    assert gen.get_diff(f1_json_p, f2_json_p, format_name='json') == tru0
    ...
    assert gen.get_diff(f1_yml_p, f2_yml_p, format_name='plain') == tru2