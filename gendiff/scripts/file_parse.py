...
def parse(f1, f2, lvl=1):
    ...
    if isinstance(f1, dict) and isinstance(f2, dict):
        ...
        for k in all_k_l:
            if k in minus:
                inter_res.append(make_diff_dict(k, 'minus', lvl, f1=f1))
            if k in plus:
                inter_res.append(make_diff_dict(k, 'plus', lvl, f2=f2))
            if k in common:
                if len(list(make_diff_dict(k, 'common', lvl, f1=f1, f2=f2).keys())) > 2:
                    inter_res.append(make_diff_dict(k, 'common', lvl, f1=f1, f2=f2))
                else:
